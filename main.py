...
async def direct_download(bot, m):
    ...
    if "b-cdn.net" in url or "mediadelivery.net" in url: headers.append('--referer "https://iframe.mediadelivery.net/"')
    elif "vimeo" in url: headers.append('--referer "https://vimeo.com/"')
    elif "shikho" in url: headers.append('--referer "https://shikho.com/"')
    header_str = " ".join(headers)
    cmd = f'yt-dlp -f "bestvideo[height<=360]+bestaudio/best" {header_str} "{url}" -o "{name}.mp4"'
    
    try:
        # এখানে msg পাস করা হয়েছে যাতে প্রোগ্রেস বার আপডেট হতে পারে
        ...
        else: await msg.edit_text("❌ ডাউনলোড ব্যর্থ।")
    except Exception as e: await msg.edit_text(f"❌ এরর: `{str(e)[:150]}`")
...
async def upload_file(bot, m):
    ...
    try:
        ...
        x = await input_msg.download(); await input_msg.delete(True)
        with open(x, "r") as f: content = f.read().split("\n")
        links = [i.split("://", 1) for i in content if "://" in i]; os.remove(x)
        ...
        for i in range(len(links)):
            try:
                ...
                cmd = f'yt-dlp -f "bestvideo[height<=360]+bestaudio/best" {h} "{u}" -o "{out}.mp4"'
    ...
    except Exception as e: await editable.edit(f"Error: {e}")
